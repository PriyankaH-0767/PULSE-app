...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    if (
      Math.abs(next.center[0] - current.center[0]) < 0.0001 &&
      Math.abs(next.center[1] - current.center[1]) < 0.0001 &&
      Math.abs(next.zoom - current.zoom) < 0.01 &&
      Math.abs(next.bearing - current.bearing) < 0.1 &&
      Math.abs(next.pitch - current.pitch) < 0.1
    ) {
      return;
    }
    ...
    // We let the internalUpdateRef stay true until moveend (handled in moveend event, or we can just let onViewportChange handle the animation)
    setTimeout(() => {
...
function ControlGroup({ children }: { children: ReactNode }) {
  ...
  return (
    <div className={cn(
      "border flex flex-col overflow-hidden rounded-md shadow-sm",
      resolvedTheme === "dark" 
        ? "bg-slate-900/80 border-white/10 backdrop-blur-md [&>button:not(:last-child)]:border-white/10" 
        : "bg-white/80 border-slate-200 backdrop-blur-md [&>button:not(:last-child)]:border-slate-200",
